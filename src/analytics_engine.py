...
class AnalyticsEngine:
    """High-performance statistical modeling & analytics engine for LuxuryBrand: High-Net-Worth VIP Clienteling & Retention Analytics."""
        ...
        with open(self.data_file_path, "r", encoding="utf-8") as f:
            ...
            for row in reader:
                self.records.append({
                    ...
                    "annual_clv_score": float(row["annual_clv_score"]),
                    "anomaly_flag": int(row["anomaly_flag"]),
                    "confidence_score": float(row["confidence_score"])
                })

    def compute_summary_statistics(self):
        vals = [r["purchase_amount"] for r in self.records]
        if not vals:
            return {}
        n = len(vals)
        mean_val = sum(vals) / n
    ...
    def detect_statistical_anomalies(self, z_threshold=2.5):
        ...
        for r in self.records:
            if std > 0:
                z = (r["purchase_amount"] - mean) / std
                if abs(z) >= z_threshold:
                    anomalies.append({
                        "record_id": r["record_id"],
                        "entity_type": r["entity_type"],
                        "value": r["purchase_amount"],
                        "z_score": round(z, 3)
                    })
        return anomalies
    ...
    def group_by_entity(self):
        ...
        for r in self.records:
            grouped[r["entity_type"]].append(r["purchase_amount"])
        result = {}
